...
{   
    ...
    {   
        ...
        {
            $products = Product::where('category_id',$category_id)->orderBy('regular_price','ASC')->paginate(10);
            $pro_total = Product::count();
        }
        elseif ($this->sorting=='price_desc') 
        {
            $products = Product::where('category_id',$category_id)->orderBy('regular_price','DESC')->paginate(10);
            $pro_total = Product::count();
        }
        else 
        {
            $products = Product::where('category_id',$category_id)->paginate(10);
            $pro_total = Product::groupBy('category_id')->count();
        }
        
        $categories = Category::all();

        return view('livewire.category-sort', ['pro_total' => $pro_total, 'products'=> $products, 'categories'=> $categories, 'category_name'=> $category_name])->layout('layouts.app');
    }
}
